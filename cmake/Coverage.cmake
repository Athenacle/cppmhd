...
if (CODE_COVERAGE AND NOT CODE_COVERAGE_ADDED)
    ...
    # Common Targets
    add_custom_target(
        ccov-preprocessing COMMAND ${CMAKE_COMMAND} -E make_directory
                                   ${CMAKE_COVERAGE_OUTPUT_DIRECTORY} DEPENDS ccov-clean)
        ...
        if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES
            ...
            add_custom_target(
                ...
                COMMAND
                    ...
                    -instr-profile=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged.profdata
                    -format="lcov" ${EXCLUDE_REGEX} > ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/lcov.info
                DEPENDS ccov-all-processing)

            add_custom_target(
                ccov-all-export-json
                COMMAND
                    ${LLVM_COV_PATH} export `cat ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list`
                    -instr-profile=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged.profdata
            ...
            add_custom_target(ccov-all-export DEPENDS ccov-all-export-lcov ccov-all-export-json)
        ...
        elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "GNU")
            ...
            add_custom_target(
                ccov-all COMMAND ${GENHTML_PATH} -o ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged
                                 ${COVERAGE_INFO} DEPENDS ccov-all-capture)
...
function (put_all_targets_into_coverage)
    cmake_parse_arguments(PUTALL "" "EXCLUDES" "ARGSLIST" ${ARGN})
    message(STATUS "EXCLUDES: ${PUTALL_EXCLUDES}")
    if (CODE_COVERAGE)
        get_all_targets(all_targets)

        foreach (T IN LISTS all_targets)
            get_target_property(PROPER ${T} TYPE)
            if (${PROPER} STREQUAL "EXECUTABLE" OR ${PROPER} MATCHES "_LIBRARY")
                target_code_coverage(${T} EXCLUDE ${PUTALL_EXCLUDES})
            endif ()
        endforeach ()
    endif ()
endfunction ()
